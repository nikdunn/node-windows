module.exports = {
   ...
   * - *stopScript* The absolute path of the node.js server script used to shut down the process gracefully.
   *
  ...
  generateXml: function(config)
  {
    ...
    multi('startargument',config.nodeOptions, ' ');
    xml.push({startargument:config.script.trim()});
    multi('startargument',config.wrapperArgs,' ');

    xml.push({stopexecutable:process.execPath});
    xml.push({stopargument:config.stopScript.trim()});
    multi('stopargument',config.stopArgs,' ');
