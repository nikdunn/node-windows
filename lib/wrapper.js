...
var Logger = require('./eventlog'),
    ...
    p = require('path'),
    fs = require('fs'),
    shutdownFile = p.join(__dirname, '.shutdown'), 
    argv = optimist
      .demand('file')
      .alias('f','file')
...
if (fs.existsSync(shutdownFile)) {
    log.info('shutdown file: ' + shutdownFile + ' already exists, deleting it');
    fs.unlinkSync(shutdownFile);
}

var chokidar = require('chokidar');

log.info('monitoring shutdown file: ' + shutdownFile);

// One-liner for current directory, ignores .dotfiles 
chokidar.watch(shutdownFile).on('add', (event, path) => {
    log.info(argv.f+' process going away because shutdown file exists: ' + shutdownFile);
